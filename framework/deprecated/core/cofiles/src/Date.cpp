...
void Date::Save( CoFiles::Serializer &clSerializer ) const
{
	clSerializer.AddU64( m_u64JulianDay );
}

void Date::Load( const CoFiles::Serializer &clSerializer )
{
	m_u64JulianDay = clSerializer.GetU64();
}
...
const Date CoFiles::operator+( const Date& _lhs, Integer64 _i64Days )
{
    Date clSum( _lhs );
    clSum += _i64Days;
    return clSum;
