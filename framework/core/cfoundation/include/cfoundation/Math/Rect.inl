...
namespace CFoundation
{
	...
	template < class T >
	void Rect< T >::SetWidth( T _tWidth )
	{
		m_vSize.SetX( _tWidth );
	}

	template < class T >
	void Rect< T >::SetHeight( T _tHeight )
	{
		m_vSize.SetY( _tHeight );
	}
	...
	{
		return Vector2D< T >( GetLeft(), GetTop() );
	}

	template <class T>
    Vector2D< T > Rect<T>::GetTopRight() const
	{
		return Vector2D< T >( GetRight(), GetTop() );
	}
    
	template <class T>
    Vector2D< T > Rect<T>::GetBottomLeft() const
	{
		return Vector2D< T >( GetLeft(), GetBottom() );
	}
    
	template <class T>
    Vector2D< T > Rect<T>::GetBottomRight() const
	{
		return Vector2D< T >( GetRight(), GetBottom() );
	}

	template < class T >
	void Rect< T >::SetSize( const Vector2D< T >& _vSize )
	{
        m_vSize = _vSize;
	}
	...
	template <class T>
	bool Rect<T>::IsPointIn( const Vector2D< T >& _vPos ) const
	{
	...
	template <class T>
	BorderType Rect<T>::GetBordersPointIsOn( const Vector2D< T > _tPos, T _tBorderWidth ) const
	{
